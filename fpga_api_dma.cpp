...
void FPGA::largeMV(const float* large_mat, const float* input, float* output, int num_input, int num_output)
{
  ...
  for(int i = 0; i < num_output; i += m_size_)
  {
    ...
    {			
      ...
      {
        float *ps_dram = mmap(NULL, sizeof(float) * block_col, PROT_READ|PROT_WRITE, MAP_SHARE, fd_, 0x10000000);
        memcpy(ps_dram, large_mat + (i + k) * num_input + j, sizeof(float) * block_col);
        ...
        *(fpga_dma + 8) = mat + k * v_size_; // should change this to adress which is seen by dma
        *(fpga_dma + 10) = sizeof(float) * block_col;
